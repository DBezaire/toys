...
class MetroTimer {
  ...
  playOneBeat(beatTime) {
    this.mtbeat.trigger(beatTime, this.beatCount % this.beatsPerMeasure, this.volume);
  ...
  start() {
    ...
    if (this.audioContext == null) {
      // Create the AudioContext and sound(s).
      // Although this is done only once per instantiation, browser privacy options 
      // require waiting until user has clicked something. 
      this.audioContext = new (window.AudioContext || window.webkitAudioContext)();
      this.mtbeat = new soundClasses.Mtbeat(this.audioContext);
    }
